...
//            }
        ...
        public void setVarValue(String value) {
            ...
            if ("true".equals(value)) {
                this.isStatic = true;
                this.result = true && !logicalNot;
            } else if ("false".equals(value)) {
                this.isStatic = true;
                this.result = false || logicalNot;
...
//                        return result = !(rightValue == Boolean.TRUE) ;
                ...
                if (evaluateEnvironment == null || evaluateEnvironment.isEmptyContext()) {
                    throw new ExpressionException("Unresolved property or variable:'" + value + "', context is null or empty !");
                }
                Object context = evaluateEnvironment.getEvaluateContext();
                Object obj = ObjectUtils.get(context, value);
                if(obj == null && !ObjectUtils.contains(context, value)) {
                    throw new ExpressionException("Unresolved property or variable:'" + value + "' by context");
                }
    ...
    protected void parse() {
        // 解析
        parseNext(exprEvaluator, false, false);
        // 置换优先级
    ...
    private void parseNext(ExprEvaluator evaluator, boolean negate, boolean logicalNot) {
        ...
        if (this.tokenType == VAR_TOKEN) {
            ...
            // 继续查找token直到终止
            parseNext(evaluator, false, false);
...
//            String token = this.tokenBuffer.toString();
            ...
            evaluator.setRight(right);
            parseNext(right, negate, logicalNot);
        } else if (this.tokenType == NEGATE_TOKEN) {
            // 负数标记，上一个token必为操作符
            // 只做桥接作用,基于负负得正与上一个negate值互斥
            parseNext(evaluator, !negate, logicalNot);
        } else if(this.tokenType == NOT_TOKEN) {
            // 取非标记，上一个token必为操作符
            // 只做桥接作用,基于`负负得正`与上一个logicalNot值互斥
            parseNext(evaluator, negate, !logicalNot);
        }
